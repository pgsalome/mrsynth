...
if __name__ == '__main__':
    ...
    mask_dir = '/media/e210-pc34/gjsalome/betmask'
    gtv_mask_dir = '/media/e210-pc34/gjsalome/trainmask' #'/media/e210-pc34/gjsalome/summamaks'  # Adjust this to the path where your GTV masks are stored
    ...
    # New dictionaries to store tumor slices
    patient_real_tumor_volumes = defaultdict(dict)
    patient_reconstructed_tumor_volumes = defaultdict(dict)
    patient_gtv_tumor_volumes = defaultdict(dict)
    ...
    for i, data in enumerate(dataset):
        ...
        # Load the corresponding 3D mask for the patient (CT1 Mask)
        ...
        ssim_value = calculate_masked_ssim(reconstructed_img, real_img, padded_mask_slice)
        ...
        # GTV Mask Processing
        gtv_mask_path = os.path.join(gtv_mask_dir, f'BraTS2021_{patient_id}_seg.nii.gz')
        ...
        # Check if the GTV mask slice contains any non-zero values (indicating a tumor presence)
        if np.any(padded_gtv_mask_slice):
            # Store the tumor slices in dictionaries with slice_index as the key
            patient_real_tumor_volumes[patient_id][slice_index] = real_img
            patient_reconstructed_tumor_volumes[patient_id][slice_index] = reconstructed_img
            patient_gtv_tumor_volumes[patient_id][slice_index] = padded_gtv_mask_slice

        # Store the GTV mask slice for 3D volume creation
        ...
        # Store the error map metrics for the patient
    ...
    for patient_id in patient_real_volumes:
        ...
        # Print the order of all images being stacked
        print(
            f"Patient {patient_id} - Stacking all image slices in order: {sorted(patient_real_volumes[patient_id].keys())}")
        ...
        # Check if there are any tumor slices for this patient
        if patient_real_tumor_volumes[patient_id]:
            # Sort tumor slices by index (assumes the slices were stored with the correct indices)
            sorted_tumor_indices = sorted(
                patient_real_tumor_volumes[patient_id].keys())  # Sort indices based on the real slices
            sorted_real_tumor_slices = [patient_real_tumor_volumes[patient_id][i] for i in sorted_tumor_indices]
            sorted_reconstructed_tumor_slices = [patient_reconstructed_tumor_volumes[patient_id][i] for i in
                                                 sorted_tumor_indices]
            sorted_gtv_tumor_slices = [patient_gtv_tumor_volumes[patient_id][i] for i in sorted_tumor_indices]

            # Print the order of tumor-only images being stacked
            print(f"Patient {patient_id} - Stacking tumor-only slices in order: {sorted_tumor_indices}")


            # Stack tumor slices to form 3D volumes
            real_tumor_volume = np.stack(sorted_real_tumor_slices, axis=-1)
            reconstructed_tumor_volume = np.stack(sorted_reconstructed_tumor_slices, axis=-1)
            gtv_tumor_volume = np.stack(sorted_gtv_tumor_slices, axis=-1)

            # Save tumor volumes as NIfTI files
            real_tumor_nifti = nib.Nifti1Image(real_tumor_volume, np.eye(4))
            reconstructed_tumor_nifti = nib.Nifti1Image(reconstructed_tumor_volume, np.eye(4))
            gtv_tumor_nifti = nib.Nifti1Image(gtv_tumor_volume, np.eye(4))

            nib.save(real_tumor_nifti, os.path.join(web_dir, patient_id, f'{patient_id}_real_tumor.nii.gz'))
            nib.save(reconstructed_tumor_nifti,
                     os.path.join(web_dir, patient_id, f'{patient_id}_reconstructed_tumor.nii.gz'))
            nib.save(gtv_tumor_nifti, os.path.join(web_dir, patient_id, f'{patient_id}_gtv_tumor.nii.gz'))
    ...
    for patient, avg_psnr in patient_avg_psnr.items():
        ...
        logger.info('Patient %s: Average PSNR = %f, Average SSIM = %f, Avg Error Map Mean = %f, Avg Error Map Std = %f',
                    patient, avg_psnr, avg_ssim, avg_error_map_mean, avg_error_map_std)
    ...
    with open(os.path.join(web_dir, 'psnr_ssim_error_map_values.txt'), 'w') as f:
        ...
        f.write('\nAverage PSNR, SSIM, and Error Map metrics for each patient:\n')
        for patient, avg_psnr in patient_avg_psnr.items():
            avg_ssim = patient_avg_ssim[patient]
            avg_error_map_mean = patient_avg_error_map_means[patient]
            avg_error_map_std = patient_avg_error_map_stds[patient]
            f.write(f'Patient {patient}: Average PSNR = {avg_psnr}, Average SSIM = {avg_ssim}, Avg Error Map Mean = {avg_error_map_mean}, Avg Error Map Std = {avg_error_map_std}\n')
