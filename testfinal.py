...
#     mask_dir = '/media/e210-pc34/Adi Hard disk/rakshana/testrough/testmask'
#
...
#         mask_path = os.path.join(mask_dir, f'Testdata_{patient_id}_CT1_mask.nii.gz')
...
if __name__ == '__main__':
    ...
    # Initialize storage for 3D volumes
    patient_real_volumes = defaultdict(list)
    patient_reconstructed_volumes = defaultdict(list)
    ...
    for i, data in enumerate(dataset):
        ...
        patient_real_volumes[patient_id].append(real_img)
        patient_reconstructed_volumes[patient_id].append(reconstructed_img)
    ...
    for patient_id in patient_real_volumes:
        real_volume = np.stack(patient_real_volumes[patient_id], axis=-1)
        reconstructed_volume = np.stack(patient_reconstructed_volumes[patient_id], axis=-1)
